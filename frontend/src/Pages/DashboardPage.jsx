    ...
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm">
        ...
        <div className="space-y-3">
            {order.items.map((item, index) => (
                <p key={index} className="text-gray-700 bg-gray-50 p-2 rounded-md">{item}</p>
            ))}
        </div>
        <div className="flex justify-between items-center mt-4 text-sm">
            <p className="font-medium">Status: <span className="text-green-600">{order.status}</span></p>
            <p className="font-medium">Total: <span className="text-blue-600">${order.total.toFixed(2)}</span></p>
        </div>
        <div className="flex gap-3 mt-4">
            <button onClick={onEdit} className="flex-1 bg-yellow-400 text-white py-2 rounded-lg hover:bg-yellow-500">Edit Order</button>
            <button onClick={onCancel} className="flex-1 bg-red-500 text-white py-2 rounded-lg hover:bg-red-600">Cancel Order</button>
        </div>
    </div>
);
    ...
    return (
        <div className="space-y-6 sm:space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
                <MenuCard menu={menu} onOrderNow={() => { setEditingOrder(null); setOrderDialogOpen(true); }} />
                {latestOrder ? (
                    <LatestOrderCard 
                        order={latestOrder}
                ...
                <h3 className="text-xl sm:text-2xl font-semibold text-gray-900">Quick Actions</h3>
                 <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <Link to="/u/orders" className="inline-flex items-center text-blue-600 hover:text-blue-700 font-medium">
                            View History
                            <svg className="w-4 h-4 ml-1" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" /></svg>
                        </Link>
                    </div>
                    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <Link to="/u/attendance" className="inline-flex items-center text-blue-600 hover:text-blue-700 font-medium">
                            View Calendar
                           <svg className="w-4 h-4 ml-1" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" /></svg>
                        </Link>
                    </div>
