...
function ResetRight() {
    ...
    const handleResetPass = async (e) => {
        ...
        try {
            ...
            toast.update(toastId, { render: response.data.message || "Password reset successfully!", type: "success", isLoading: false, autoClose: 3000 });
            navigate("/login");
        } catch (err) {
            console.error(err);
            toast.update(toastId, { render: err.response?.data?.message || "An error occurred. Please try again.", type: "error", isLoading: false, autoClose: 3000 });
        }
    };
